...
function printRecords(recordIds) {
  const recordsEnrolled = studentRecords.filter(function isStudentEnrolled(
    record
  ) {
    return recordIds.includes(record.id);
  });
  recordsEnrolled.sort(sortByNameAsc);
  recordsEnrolled.forEach(printRecord);
}
...
function recordsPaidAndNotEnrolled() {
  const recordsFiltered = studentRecords.filter(function idsPaidAndNotEnrolled(
    record
  ) {
...
function recordsEnrolled() {
  const recordsFiltered = studentRecords.filter(function isEnrolledStudent(
    record
  ) {
...
function isUnpaidStudent(record) {
  return !record.paid;
}

function remindUnpaid(recordIds) {
  const unpaidStudents = studentRecords.filter(isUnpaidStudent);
